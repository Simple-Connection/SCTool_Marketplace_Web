...
def classify_claim(claim,declared_namespace=None):
    ...
    if match and reg["namespaces"].get(match["namespace"],{}).get("routable"):
        ...
        if declared_namespace and actual!=declared_namespace:
            ...
            result={"status":status,"primary":actual,"variant":match["variant_id"],"declared_namespace":declared_namespace,"actual_namespace":actual,"missing":[],"extra":[]}
            if ex: result["exclusion_rules"]=ex
            return result
        return {"status":"CLASSIFIED","primary":actual,"variant":match["variant_id"],"missing":[],"extra":[]}
