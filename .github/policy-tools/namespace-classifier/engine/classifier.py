...
REGISTRY=build_registry(); FIELDS=REGISTRY["claim_fields"]
INDEX={}
for v in REGISTRY["variants"]: INDEX.setdefault(tuple(v[f] for f in FIELDS),[]).append(v)

def boundary_match(declared,claim,actual=None):
    b=REGISTRY.get("namespace_boundaries",{}).get(declared,{})
    out=[]
    for r in b.get("exclusion_rules",[]):
        if actual and actual in r.get("namespaces",[]): out.append(r["id"]); continue
        if claim.get("operation") in r.get("operations",[]): out.append(r["id"]); continue
        if claim.get("effect") in r.get("effects",[]): out.append(r["id"])
...
def classify_claim(claim,declared_namespace=None):
    ...
    if len(matches)==1:
        ...
        if declared_namespace and actual!=declared_namespace:
            ...
            if ex: result["exclusion_rules"]=ex
            return result
        return {"status":"CLASSIFIED","primary":actual,"variant":v["variant_id"],"missing":[],"extra":[]}
    if len(matches)>1: return {"status":"AMBIGUOUS","primary":None,"variant":None,"candidates":sorted({m["namespace"] for m in matches}),"missing":[],"extra":[]}
...
def validate_edge(edge):
    ...
    src,tgt,rel=edge["from"],edge["to"],edge["relation"]; ns=REGISTRY["namespace_boundaries"]; c=REGISTRY["cross_namespace_edges"].get(rel)
    ...
    if "*" not in c["allowed_to"] and tgt not in c["allowed_to"]: return {"status":"INVALID_EDGE","reason":"TARGET_NAMESPACE_NOT_ALLOWED","edge":edge}
...
def classify_policy(policy):
    ...
    if declared is not None and declared not in REGISTRY["namespace_boundaries"]: return {"status":"UNKNOWN_DECLARED_NAMESPACE","primary":None,"namespaces":[],"claim_results":[],"edge_results":[]}
    ...
    for s in priority:
        if s in statuses: return {"status":s,"primary":None,"namespaces":namespaces,"claim_results":results,"edge_results":edge_results}
    if any(e["status"]!="VALID" for e in edge_results): return {"status":"INVALID_EDGE","primary":None,"namespaces":namespaces,"claim_results":results,"edge_results":edge_results}
    if len(namespaces)==1: return {"status":"CLASSIFIED","primary":namespaces[0],"namespaces":namespaces,"claim_results":results,"edge_results":edge_results}
